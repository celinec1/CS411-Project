...
def submit():
    ...
    query = {'user_id': user_id}  # Provide the query to identify the document to update
    data = {'location': location, 'destination': destination}  # Add the new key-value pairs

    insert_data(collection, query, data)
    ...
    if response.status_code == 201:
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 201:
                ...
                query = {'user_id': user_id}  # Provide the query to identify the document to update
                data = {'Playlist': link}  # Add the new key-value pairs
